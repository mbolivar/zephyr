...
def write_props(node):
    ...
    def write_prop_primary(ident, prop):
        ...
        # - arrays involving phandles have special cases depending on
        #   the length of the array (adding a trailing _0 or not)
        # - phandle-array values have unnecessary special cases which can't be
        #   fixed within the same namespace
        #
        ...
        if prop.type in ["boolean", "string", "int"]:
            ...
            # Array types that don't involve phandles are handled by
...
def write_controller_and_data_other(ident, prop_ident, entry, suffix,
    ...
    for cell_i, cell in enumerate(entry.data.keys()):
        cell_ident = str2ident(cell)
        cell_macro = out(f"{ident}_{prop_ident}_{cell_ident}{suffix}",
                         cell_macros[cell_i])
        if entry_ident:
...
def write_bus(node):
    ...
    def write_bus_node_for_ident(node, ident):
        ...
        if primary_macro is None:
            # FIXME the str2ident call here is wrong. But it's matching
            # the old implementation exactly. It should be removed.
